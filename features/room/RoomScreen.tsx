...
export default function Home() {
  ...
  const { tasks: relatedTasks, pendingTaskIds, setTaskStatus } = useRoomRelatedTasks(selected && isRoomItemId(selected.id) ? selected.id : null);
  ...
  return <main className="shell">
    ...
    {selected&&<div className="overlay" onMouseDown={()=>setSelected(null)}><section className="sheet" role="dialog" aria-modal="true" aria-labelledby="dialog-title" onMouseDown={event=>event.stopPropagation()}><div className="grabber"/><p className="eyebrow">Cambiar estado</p><h2 id="dialog-title">{selected.name}</h2><p className="last-updated">Última actualización: <b>{formatRoomUpdate(getLastRoomUpdate(selected.id,updated))}</b></p>{ROOM_CHILDREN[selected.id as RoomZoneId]&&<p className="parent-note">Al cambiar esta zona, se actualizarán todas sus subzonas.</p>}<div className="options">{(['ok','review','attention'] as RoomStatus[]).map(status=><button key={status} className={currentSelected===status?'active':''} onClick={()=>setStatus(status)}><Dot status={status}/><span><b>{ROOM_STATUS_LABEL[status]}</b><small>{status==='ok'?'Todo está en su lugar':status==='review'?'Conviene darle un vistazo':'Necesita ordenarse'}</small></span><i>{currentSelected===status?'✓':''}</i></button>)}</div>{isRoomItemId(selected.id)&&<div className="room-related-tasks"><div className="room-related-tasks__heading"><b>Tareas vinculadas</b><button onClick={()=>{setSelected(null);requestTaskCreationForRoom(selected.id)}}>+ Agregar tarea</button></div>{relatedTasks.length===0?<p className="room-related-tasks__empty">No hay tareas para este elemento.</p>:<div className="room-related-tasks__list">{relatedTasks.map(task=><article key={task.id} className={`room-related-task${task.status === 'completed' ? ' completed' : ''}`}><TaskStatusButton task={task} pending={pendingTaskIds.has(task.id)} onToggle={() => { void setTaskStatus(task, task.status === 'completed' ? 'pending' : 'completed', goals).catch(() => {}); }}/><button onClick={()=>{setSelected(null);requestTaskEdit(task.id)}}><span>{task.title}</span><b aria-hidden="true">›</b></button></article>)}</div>}</div>}<button className="cancel" onClick={()=>setSelected(null)}>Cancelar</button></section></div>}
