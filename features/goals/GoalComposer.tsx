...
export function GoalComposer({ open, goal, onClose }: { open: boolean; goal: Goal | null; onClose: () => void }) {
  const { repository } = useGoals(); const { projects } = useProjects(); const { tasks } = useTasks(); const [input, setInput] = useState<GoalInput>(blank); const [error, setError] = useState('');
  const linkedTaskCount = goal ? tasks.filter((task) => task.goal_id === goal.id).length : 0;
